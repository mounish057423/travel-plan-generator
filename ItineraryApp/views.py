...
genai.configure(api_key="AIzaSyBPDe6WxD39iNdkZa7Ev4nWmkvdRJg1nmI")
...
def loadData():
    ...
    flag = False
    for rood, dirs, directory in os.walk('ItineraryApp/static/model'):
        for j in range(len(directory)):
            if directory[j].lower() not in Y:
                with open(rood+"/"+directory[j], "rb") as file:
                    data = file.read()
                file.close()
                data = data.decode()
                data = data.strip('\n').strip().lower() 
...
def TravelPlanAction(request):
    if request.method == 'POST':
        ...
        for rood, dirs, directory in os.walk('ItineraryApp/static/location_images/'+destination.lower()):
            for j in range(len(directory)):
...
def UserLoginAction(request):
    if request.method == 'POST':
        ...
        index = 0
        con = pymysql.connect(host='127.0.0.1',port = 3306,user = 'root', password = 'rood', database = 'itinerary',charset='utf8')
        with con:    
            cur = con.cursor()
        ...
        with con:    
            ...
            for row in rows:
                if row[0] == username:
                    ...
                    break
        if status == "none":
            db_connection = pymysql.connect(host='127.0.0.1',port = 3306,user = 'root', password = 'rood', database = 'itinerary',charset='utf8')
